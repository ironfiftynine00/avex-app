...
import { insertCategorySchema, insertQuestionSchema, insertExamAttemptSchema, insertStudySessionSchema, infographics, categorySubtopics, battleParticipants, questions, subtopics, categories } from "@shared/schema";
import { db } from "./db";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/categories/:categoryId/subtopics', isAuthenticated, async (req, res) => {
    try {
      const categoryId = parseInt(req.params.categoryId);
      const subtopics = await storage.getSubtopicsByCategory(categoryId);
      res.json(subtopics);
      ...
      res.status(500).json({ message: "Failed to fetch subtopics for category" });
    }
  });
