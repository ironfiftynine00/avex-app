import {
  ...
  type PracticalStation,
  type InsertPracticalStation,
  type PracticalContent,
  type InsertPracticalContent,
} from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  // Admin Analytics
  getRealtimeUserActivity(): Promise<any[]>;
  getRecentExamAttempts(limit: number): Promise<any[]>;
  getRecentStudySessions(limit: number): Promise<any[]>;
  getActivityOverview(): Promise<any>;
  
  // Practical Study Guide Content
  getPracticalStations(): Promise<PracticalStation[]>;
  getPracticalStation(id: number): Promise<PracticalStation | undefined>;
  createPracticalStation(station: InsertPracticalStation): Promise<PracticalStation>;
  updatePracticalStation(id: number, updates: Partial<InsertPracticalStation>): Promise<PracticalStation>;
  deletePracticalStation(id: number): Promise<void>;
  
  getPracticalContentByStation(stationId: number): Promise<PracticalContent[]>;
  getPracticalContent(id: number): Promise<PracticalContent | undefined>;
  createPracticalContent(content: InsertPracticalContent): Promise<PracticalContent>;
  updatePracticalContent(id: number, updates: Partial<InsertPracticalContent>): Promise<PracticalContent>;
  deletePracticalContent(id: number): Promise<void>;
  bulkCreatePracticalContent(stationId: number, contents: Omit<InsertPracticalContent, 'stationId'>[]): Promise<PracticalContent[]>;
  reorderPracticalContent(stationId: number, contentIds: number[]): Promise<void>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteUser(id: string): Promise<void> {
    ...
    // Start a transaction to ensure all deletions complete or none do
    await db.transaction(async (tx) => {
      // Delete all user-related records in the correct order (dependent tables first)
      
      // Battle-related data
      await tx.delete(battleEvents).where(eq(battleEvents.userId, userId));
      await tx.delete(battleAnswers).where(eq(battleAnswers.userId, userId));
      await tx.delete(battleParticipants).where(eq(battleParticipants.userId, userId));
      
      // Delete battle rooms where user is host (these will cascade to related battle data)
      await tx.delete(battleRooms).where(eq(battleRooms.hostUserId, userId));
      
      // User progress and analytics data
      await tx.delete(userCategoryStats).where(eq(userCategoryStats.userId, userId));
      await tx.delete(userAnalytics).where(eq(userAnalytics.userId, userId));
      await tx.delete(userSubtopicProgress).where(eq(userSubtopicProgress.userId, userId));
      await tx.delete(userQuestionCompletion).where(eq(userQuestionCompletion.userId, userId));
      await tx.delete(userProgress).where(eq(userProgress.userId, id));
      
      // Study and exam data
      await tx.delete(studySessions).where(eq(studySessions.userId, id));
      await tx.delete(examAttempts).where(eq(examAttempts.userId, id));
  ...
  async trackQuestionCompletion(userId: number, questionId: number, isCorrect: boolean, studyMode: 'practice' | 'quiz' | 'exam'): Promise<number> {
    try {
      ...
      if (existing.length > 0) {
        // Already tracked this question, no new progress added
        return 0;
      }
      ...
      // Insert new completion record - track both correct and incorrect answers
      // Only set firstCorrectAnswer timestamp if the answer was correct
      await db.insert(userQuestionCompletion).values({
        userId,
        questionId,
        firstCorrectAnswer: isCorrect ? sql`NOW()` : null,
        progressPercentage: progressPercentage.toFixed(6),
        studyMode,
      });

      console.log(`User ${userId} ${isCorrect ? 'correctly answered' : 'attempted'} question ${questionId} for ${progressPercentage.toFixed(4)}% progress in ${studyMode} mode`);
  ...
  async getRandomQuestions(categoryId: number, count: number): Promise<Question[]> {
    try {
      // First, get all question IDs for the category (faster than full join with RANDOM())
      ...
      // Shuffle the IDs in memory (much faster than SQL RANDOM())
      const shuffledIds = questionIds
        .map(q => q.id)
        .sort(() => Math.random() - 0.5)
        .slice(0, Math.min(count, questionIds.length));
      
      // Now fetch the full question data for only the selected IDs
      const randomQuestions = await db
        ...
        .from(questions)
        .where(inArray(questions.id, shuffledIds));
      
      // Maintain the shuffled order
      const orderedQuestions = shuffledIds.map(id => 
        randomQuestions.find(q => q.id === id)
      ).filter(Boolean) as typeof randomQuestions;
      
      console.log(`Successfully fetched ${orderedQuestions.length} random questions for category ${categoryId}`);
      return orderedQuestions;
    } catch (error) {
      console.error(`Error fetching random questions for category ${categoryId}:`, error);
      // Fallback to simpler query if optimization fails
      return await db
        .select({
          id: questions.id,
          subtopicId: questions.subtopicId,
  ...
  async getUserExamAttempts(userId: string): Promise<ExamAttempt[]> {
    return await db
      .select({
        ...
        questionsData: examAttempts.questionsData,
        createdAt: examAttempts.createdAt,
        timeSpent: examAttempts.timeSpent,
        questionsAnswered: examAttempts.questionsAnswered,
        questionsSkipped: examAttempts.questionsSkipped,
        categoryName: categories.name, // Get real category name from categories table
        answers: examAttempts.answers,
      })
      .from(examAttempts)
      .leftJoin(categories, eq(examAttempts.categoryId, categories.id))
  ...
  async updateAnalyticsOnActivity(
    ...
    if (data.categoryId && activityType === 'mock_exam') {
      ...
      // Calculate category-specific totals from user_question_completion table
      const categoryQuestionCompletions = await db
        .select({
          totalAnswered: sql<number>`COUNT(*)`,
          totalCorrect: sql<number>`SUM(CASE WHEN ${userQuestionCompletion.firstCorrectAnswer} IS NOT NULL THEN 1 ELSE 0 END)`
        })
        .from(userQuestionCompletion)
        .innerJoin(questions, eq(userQuestionCompletion.questionId, questions.id))
        .innerJoin(categorySubtopics, eq(questions.subtopicId, categorySubtopics.subtopicId))
        .where(and(
          eq(userQuestionCompletion.userId, userId),
          eq(categorySubtopics.categoryId, data.categoryId)
        ));

      const actualCategoryQuestionsAnswered = categoryQuestionCompletions[0]?.totalAnswered || 0;
      const actualCategoryCorrectAnswers = categoryQuestionCompletions[0]?.totalCorrect || 0;
      ...
      const updateData: Partial<UserCategoryStats> = {
        ...
        questionsAnswered: actualCategoryQuestionsAnswered,
        correctAnswers: actualCategoryCorrectAnswers,
      };
    ...
    // Calculate correct totals from user_question_completion table to avoid accumulation issues
    const questionCompletions = await db
      .select({
        totalAnswered: sql<number>`COUNT(*)`,
        totalCorrect: sql<number>`SUM(CASE WHEN ${userQuestionCompletion.firstCorrectAnswer} IS NOT NULL THEN 1 ELSE 0 END)`
      })
    ...
    const actualQuestionsAnswered = questionCompletions[0]?.totalAnswered || 0;
    const actualCorrectAnswers = questionCompletions[0]?.totalCorrect || 0;
  ...
  // Session tracking methods for analytics
  ...
  async getUserSessions(userId: number, limit: number = 30): Promise<UserSession[]> {
    return await db
      .select()
      .from(userSessions)
  ...
  async getUserSessionsByDateRange(userId: number, startDate: Date, endDate: Date): Promise<UserSession[]> {
    return await db
      .select()
      .from(userSessions)
      .where(and(
        eq(userSessions.userId, userId),
        sql`${userSessions.startTime} >= ${startDate}`,
        sql`${userSessions.startTime} <= ${endDate}`
      ))
      .orderBy(desc(userSessions.startTime));
  }

  async getUserDailyScreenTime(userId: number, days: number = 30): Promise<Array<{
    date: string;
    totalTime: number;
    sessionCount: number;
    ...
    const sessions = await db
      .select({
        ...
        activityType: userSessions.activityType,
      })
      .from(userSessions)
      .where(and(
        eq(userSessions.userId, userId),
        sql`${userSessions.startTime} >= ${startDate}`,
        eq(userSessions.isActive, false) // Only completed sessions
      ))
      .orderBy(desc(sql`DATE(${userSessions.startTime})`));
    ...
    sessions.forEach(session => {
      ...
      current.totalTime += session.duration || 0;
      current.sessionCount += 1;
      current.activityTypes.add(session.activityType || 'general');
  ...
  // Single session management methods
  async hasActiveSession(userId: number): Promise<boolean> {
    // Clean up sessions inactive for more than 15 minutes (session timeout)
    ...
    await db
      ...
      .where(and(
        eq(userSessions.userId, userId),
        eq(userSessions.isActive, true),
        sql`${userSessions.lastActivityAt} < ${fifteenMinutesAgo}`
      ));

    // Check for truly active sessions (active within last 15 minutes)
  ...
  async terminateAllUserSessions(userId: number): Promise<void> {
    ...
    // Get all active sessions for duration calculation
    const activeSessions = await db
      .select()
      .from(userSessions)
      .where(and(
  ...
  async createAuthSession(userId: number, authSessionId: string, deviceInfo?: string): Promise<UserSession> {
  ...
  async validateSessionById(sessionId: string): Promise<UserSession | null> {
    const sessions = await db
      .select()
      .from(userSessions)
      .where(and(
  ...
  async getRealtimeUserActivity(): Promise<any[]> {
    try {
      ...
      console.error('Error fetching realtime user activity:', error);
      return [];
    }
  }

  async getRecentExamAttempts(limit: number = 50): Promise<any[]> {
    try {
      const exams = await db
        .select({
          ...
          email: users.email,
          categoryId: examAttempts.categoryId,
          categoryName: examAttempts.categoryName,
          score: examAttempts.score,
          totalQuestions: examAttempts.totalQuestions,
          correctAnswers: examAttempts.correctAnswers,
          timeTaken: examAttempts.timeTaken,
      ...
      console.error('Error fetching recent exam attempts:', error);
      return [];
    }
  }

  async getRecentStudySessions(limit: number = 50): Promise<any[]> {
    try {
      const sessions = await db
        .select({
          id: studySessions.id,
          ...
          lastName: users.lastName,
          email: users.email,
          categoryId: studySessions.categoryId,
          categoryName: categories.name,
          subtopicId: studySessions.subtopicId,
        ...
        .leftJoin(categories, eq(studySessions.categoryId, categories.id))
        .leftJoin(subtopics, eq(studySessions.subtopicId, subtopics.id))
        .orderBy(desc(studySessions.createdAt))
        .limit(limit);
  ...
  async getComprehensiveUserHistory(): Promise<any[]> {
    try {
      // Get comprehensive user activity combining exams and study sessions
      const userHistory = await db
        .select({
          ...
          lastName: users.lastName,
          email: users.email,
          activityType: sql<string>`'exam'`,
          categoryName: examAttempts.categoryName,
          subtopicName: sql<string>`NULL`,
          score: examAttempts.score,
          totalQuestions: examAttempts.totalQuestions,
          correctAnswers: examAttempts.correctAnswers,
          passed: examAttempts.passed,
          ...
          questionsAnswered: sql<number>`NULL`,
          createdAt: examAttempts.createdAt
        })
        .from(examAttempts)
        .innerJoin(users, eq(examAttempts.userId, users.id))
        .unionAll(
          db.select({
            ...
            lastName: users.lastName,
            email: users.email,
            activityType: sql<string>`COALESCE(${studySessions.sessionType}, 'study')`,
            categoryName: sql<string>`COALESCE(${categories.name}, 'General Study')`,
            subtopicName: sql<string>`${subtopics.name}`,
            score: sql<string>`CASE WHEN ${studySessions.questionsAnswered} > 0 THEN ROUND((${studySessions.correctAnswers}::float / ${studySessions.questionsAnswered}) * 100)::text ELSE NULL END`,
            totalQuestions: sql<number>`NULL`,
            correctAnswers: sql<number>`COALESCE(${studySessions.correctAnswers}, 0)`,
            passed: sql<boolean>`NULL`,
            timeTaken: sql<number>`NULL`,
            duration: sql<number>`COALESCE(${studySessions.duration}, 0)`,
            questionsAnswered: sql<number>`COALESCE(${studySessions.questionsAnswered}, 0)`,
            createdAt: studySessions.createdAt
          })
          .from(studySessions)
          .innerJoin(users, eq(studySessions.userId, users.id))
      ...
      console.error('Error fetching comprehensive user history:', error);
      return [];
    }
  }
    ...
    try {
      const [
        ...
        db.select({ count: count() })
          .from(userSessions)
          .where(sql`${userSessions.lastActivityAt} > NOW() - INTERVAL '24 HOURS'`),
        
        // Total exam attempts
        db.select({ count: count() }).from(examAttempts),
        
        // Total study sessions
        db.select({ count: count() }).from(studySessions),
        
        // Average exam score
        db.select({ avgScore: sql`AVG(${examAttempts.score})` }).from(examAttempts),
      ...
      return {
        ...
        totalStudySessions: totalStudySessions[0]?.count || 0,
        averageScore: parseFloat(String(avgScore[0]?.avgScore) || '0'),
  ...
  // Practical Study Guide Content Management
  async getPracticalStations(): Promise<PracticalStation[]> {
    return await db.select().from(practicalStations)
      .where(eq(practicalStations.isActive, true))
      .orderBy(practicalStations.category, practicalStations.name);
  }

  async getPracticalStation(id: number): Promise<PracticalStation | undefined> {
    const result = await db.select().from(practicalStations)
      .where(eq(practicalStations.id, id));
    return result[0];
  }

  async createPracticalStation(station: InsertPracticalStation): Promise<PracticalStation> {
    const [newStation] = await db.insert(practicalStations)
      .values({
        ...station,
        updatedAt: new Date()
      })
  ...
  async updatePracticalStation(id: number, updates: Partial<InsertPracticalStation>): Promise<PracticalStation> {
    const [updatedStation] = await db.update(practicalStations)
      .set({
        ...updates,
        updatedAt: new Date()
      })
  ...
  async deletePracticalStation(id: number): Promise<void> {
    await db.delete(practicalStations)
      .where(eq(practicalStations.id, id));
  }

  async getPracticalContentByStation(stationId: number): Promise<PracticalContent[]> {
    return await db.select().from(practicalContent)
      .where(and(
        eq(practicalContent.stationId, stationId),
        eq(practicalContent.isVisible, true)
      ))
      .orderBy(practicalContent.orderIndex);
  }

  async getPracticalContent(id: number): Promise<PracticalContent | undefined> {
    const result = await db.select().from(practicalContent)
      .where(eq(practicalContent.id, id));
    return result[0];
  }

  async createPracticalContent(content: InsertPracticalContent): Promise<PracticalContent> {
    const [newContent] = await db.insert(practicalContent)
      .values({
        ...content,
        updatedAt: new Date()
      })
  ...
  async updatePracticalContent(id: number, updates: Partial<InsertPracticalContent>): Promise<PracticalContent> {
    // Filter out undefined/null values to prevent overwriting existing data
    ...
    // Never allow stationId to be changed through updates for security
    delete definedUpdates.stationId;
    delete definedUpdates.id;

    const [updatedContent] = await db.update(practicalContent)
      .set({
        ...definedUpdates,
        updatedAt: new Date()
      })
  ...
  async deletePracticalContent(id: number): Promise<void> {
    await db.delete(practicalContent)
      .where(eq(practicalContent.id, id));
  }

  async bulkCreatePracticalContent(stationId: number, contents: Omit<InsertPracticalContent, 'stationId'>[]): Promise<PracticalContent[]> {
    if (contents.length === 0) return [];
    
    const contentsWithStation = contents.map((content, index) => ({
      ...content,
      stationId,
      orderIndex: content.orderIndex || index,
      updatedAt: new Date()
    }));

    return await db.insert(practicalContent)
      .values(contentsWithStation)
      .returning();
  }

  async reorderPracticalContent(stationId: number, contentIds: number[]): Promise<void> {
    for (let i = 0; i < contentIds.length; i++) {
      await db.update(practicalContent)
        .set({ 
          orderIndex: i,
          updatedAt: new Date()
        })
        .where(and(
          eq(practicalContent.id, contentIds[i]),
          eq(practicalContent.stationId, stationId)
        ));
    }
  }
