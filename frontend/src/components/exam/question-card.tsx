...
interface QuestionCardProps {
  ...
  answeredCount?: number; // Number of questions actually answered (not skipped)
  isSkipped?: boolean; // Whether current question was skipped before
}
...
export default function QuestionCard({
  ...
  // For Quiz/Mock Exam mode, use answeredCount for progress; otherwise use questionNumber
  const progressCount = answeredCount !== undefined ? answeredCount : questionNumber;
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">
              {answeredCount !== undefined ? (
                // Quiz/Mock Exam mode: Hide question counter, only show revisiting status
                <>
                  {isSkipped && <span className="text-amber-600">Revisiting Skipped Question</span>}
                </>
              ) : (
                // Traditional mode: Show question position
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">
                {cumulativePercentage !== undefined ? (
                  // Quiz/Mock Exam mode: Show score based on answered questions only
                  <>Score: {currentScore}/{answeredCount || 0} ({Math.round(scorePercentage)}%)</>
                ) : (
          ...
          {/* Show unanswered questions reminder for Mock Exam */}
          {unansweredQuestions !== undefined && unansweredQuestions > 0 && (
            <div className="flex items-center justify-center bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-2 mt-2">
              <span className="text-xs text-amber-700 dark:text-amber-300 font-medium">
                📝 {unansweredQuestions} question{unansweredQuestions !== 1 ? 's' : ''} remaining to complete exam
              </span>
            </div>
          )}
          
          {/* Show special message for revisiting skipped questions */}
          {isSkipped && (
            <div className="flex items-center justify-center bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-2 mt-2">
              <span className="text-xs text-blue-700 dark:text-blue-300 font-medium">
                🔄 This question was skipped earlier - please answer it now to continue
              </span>
            </div>
          )}
        </div>
          ...
          {isPracticeMode ? (
            ...
            <>
              <div className="flex items-center space-x-3">
                {showSkipButton && skipsRemaining > 0 && !isSkipped && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={onSkip}
              ...
              <div className="flex items-center space-x-3">
                ...
                <Button 
                  onClick={(answeredCount !== undefined && answeredCount === totalQuestions) ? (onSubmit || onNext) : onNext}
                  disabled={!answer}
                  className="avex-button-primary"
                >
                  {/* Show Submit Exam only when ALL questions are already answered (30/30) */}
                  {(answeredCount !== undefined && answeredCount === totalQuestions) ? "Submit Exam" : "Next Question"}
                </Button>
              </div>
            </>
          )}
