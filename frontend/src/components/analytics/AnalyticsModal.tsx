...
export function AnalyticsModal({ open, onOpenChange }: AnalyticsModalProps) {
  ...
  const overallProgress = Number(overview.overallProgress || 0);
  ...
  const studyProgress = overallProgress;
              ...
              <CardContent>
                <div className="space-y-4 max-h-60 overflow-y-auto">
                  {analytics.categoryStats.map((stat) => {
                    ...
                    // Calculate realistic progress based on actual questions answered relative to category size
                    // Assume each category has approximately 200-300 questions for realistic progress
                    const estimatedCategorySize = 250;
                    const questionsProgress = Math.min((stat.questionsAnswered / estimatedCategorySize) * 100, 100);
                    // Weight progress by accuracy to reflect actual learning
                    const accuracyWeight = accuracy / 100;
                    const categoryProgress = Math.min(questionsProgress * accuracyWeight, 100);
                    return (
                      <div key={stat.categoryId} className="space-y-2">
                        ...
                        <div className="flex justify-between text-xs text-muted-foreground">
                          <span>
                            ...
                            {stat.questionsAnswered > 0 && stat.bestScore > 0 && (
