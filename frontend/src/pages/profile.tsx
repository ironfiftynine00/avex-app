            ...
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div>
                    <div className="text-2xl font-bold text-avex-blue">{streak?.streak || 0}</div>
                    <div className="text-xs text-muted-foreground">Day Streak</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-avex-indigo">{userBadges?.length || 0}</div>
                  ...
                  <CardContent>
                    {examAttempts?.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {examAttempts?.slice(0, 5).map((attempt: any) => {
          ...
          <TabsContent value="progress" className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {CATEGORIES.map((category) => {
                const categoryProgress = progress?.find((p: any) => p.categoryId === category.id);
                const progressPercent = categoryProgress 
                  ? (categoryProgress.correctAnswers / categoryProgress.totalAttempts * 100) || 0
                  : 0;
                
                const categoryAttempts = examAttempts?.filter((exam: any) => exam.categoryId === category.id) || [];
          ...
          <TabsContent value="settings" className="mt-6">
            <div className="max-w-2xl space-y-6">
              ...
              <Card className="avex-card">
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    <User className="w-5 h-5 text-avex-blue" />
