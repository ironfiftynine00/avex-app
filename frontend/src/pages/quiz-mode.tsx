...
export default function QuizMode() {
  ...
  const handleNextQuestion = () => {
    ...
    const findNextQuestionIndex = () => {
      ...
      for (let i = quizState.currentQuestionIndex + 1; i < quizState.questions.length; i++) {
        if (!(i in quizState.answers) && !quizState.skippedQuestions.includes(i)) {
          return i;
        }
      }
      ...
      if (quizState.skippedQuestions.length > 0) {
        ...
        for (const skippedIndex of quizState.skippedQuestions) {
          if (!(skippedIndex in quizState.answers)) {
            return skippedIndex;
          }
        }
      }
  ...
  const handleSubmitQuiz = () => {
    ...
    const quizData = {
      ...
      questionsSkipped: quizState.originallySkippedQuestions.length,
      studyMode: 'quiz'
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950/20 dark:to-amber-950/20">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <QuestionCard
          ...
          unansweredQuestions={quizState.questions.length - Object.keys(quizState.answers).length}
          answeredCount={Object.keys(quizState.answers).length}
          isSkipped={quizState.originallySkippedQuestions.includes(quizState.currentQuestionIndex)}
        />
      </div>
